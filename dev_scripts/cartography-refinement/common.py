...
BASE_SHA = "eb5e06ab4aab732572e6e46aafcfba5acea9b6bc"
BRANCH = "planet-cartography-refinement"
...
def complete(number: int, script: str, details: dict):
    ...
    if state.get("completed") != expected: raise RuntimeError(f"phase {number} requires completed={expected}; actual={state.get('completed')}")
