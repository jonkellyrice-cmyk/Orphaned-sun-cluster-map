...
import { BODY_OPERATIONS_MODEL_VERSION, BODY_OPERATIONS_SCHEMA_VERSION, BODY_OPERATIONS_STATUS, bodyOperationAssetPath, canonicalSourceFingerprint, coordinateFrameForKind, deriveBodyOperationTargets, operationLayerDefinitions, operationSeed, operationalKindForRow, stableUnit } from "../scripts/body-operations.mjs";
...
export function generateBodyOperationAsset(row) {
  ...
  const asset={ schemaVersion:BODY_OPERATIONS_SCHEMA_VERSION, system:row.system, body:row.object, canonicalType:operationalKind, operationalKind, permanentOperationalSeed:operationSeed(row.system,row.object,operationalKind), operationalModelVersion:BODY_OPERATIONS_MODEL_VERSION, canonicalSourceFingerprint:canonicalSourceFingerprint(row.system,row.object,operationalKind), acceptedCanonStatus:BODY_OPERATIONS_STATUS, coordinateFrame:coordinateFrameForKind(operationalKind), namingProfile:PROFILE[row.system]??"union", layers:operationLayerDefinitions(operationalKind), features:[], operationalSummary:{} };
